...
from .services import build_dashboard_payload, pdf_response, xlsx_response
...
class AdminDashboardView(APIView):
    ...
    def get(self, request):
        return Response(build_dashboard_payload(request), status=status.HTTP_200_OK)


"""
ADMIN DASHBOARD EXPORT VIEW
"""
class AdminDashboardExportView(APIView):
    ...
    def get(self, request):
        ...
        if export_format == "pdf":
            return pdf_response(payload, f"{filename_base}.pdf")
        return xlsx_response(payload, f"{filename_base}.xlsx")
