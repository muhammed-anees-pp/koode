    ...
    return (
        <div className="flex flex-col h-full bg-[#0d111d] border-l border-slate-700/60">
            ...
            <div className="p-3 border-t border-slate-700/50 flex items-center gap-2 bg-[#111827]">
                ...
                />
                <button
                    onClick={() => { const val = inputRef.current?.value?.trim(); if (val) { onSend(val); inputRef.current.value = ""; } }}
...
export default function PsychologistInterviewModal({ interviewId, onLeave }) {
    ...
    const createFakeStream = useCallback((userName) => {
        ...
        const draw = () => {
            ...
            ctx.strokeStyle = `rgba(17, 136, 216, ${pulse * 0.4})`;
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(320, 155, 72 + pulse * 6, 0, Math.PI * 2);
            ctx.stroke();
            ctx.fillStyle = "#1188d8";
            ctx.beginPath();
            ctx.arc(320, 155, 68, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#fff";
            ctx.font = "bold 54px system-ui, sans-serif";
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText(initial, 320, 158);
            ...
            ctx.beginPath();
            ctx.arc(300, 320, 5, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#ef4444";
            ctx.font = "bold 11px system-ui, sans-serif";
            ctx.fillText("LIVE", 315, 324);
        };

        draw();
        const rafHandle = { id: null };
        const loop = () => { draw(); rafHandle.id = requestAnimationFrame(loop); };
        rafHandle.id = requestAnimationFrame(loop);
    ...
    const joinRoom = useCallback(async (td) => {
        ...
        try {
            ...
            try {
                ...
                console.warn("Camera+mic failed, trying audio-only:", err1);
                ...
                try {
                    ...
                    console.warn("Audio-only also failed, trying mic via getUserMedia:", err2);

                    let audioTrackForCustom = null;
                    try {
                    ...
                    zegoStream = await engine.createStream({
                        custom: {
                            videoTrack: fake.videoTrack,
                            audioTrack: audioTrackForCustom || fake.audioTrack,
                        }
                    });
                    streamMode = "canvas_avatar";
                    setCamOn(false);
                    if (!audioTrackForCustom) setMicOn(false);
                    setMediaWarning(
                        audioTrackForCustom
                            ? "Camera unavailable (in use by Brave/admin). Showing avatar — your microphone is active."
                            : "Camera & mic unavailable. Showing avatar with silent audio. Both sides are still connected."
                    );
                }
            }
            ...
            console.error("Psychologist join room error:", err);
            setError("Could not connect to interview room: " + (err?.message || "Unknown error"));
            setLoading(false);
        }
        ...
        return (
            <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black">
                <div className="text-center">
                    ...
                    <button onClick={onLeave} className="px-5 py-2.5 bg-slate-700 text-white text-sm font-medium rounded-xl border-none cursor-pointer hover:bg-slate-600 transition-all">
                        Go Back
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="fixed inset-0 z-[200] flex bg-black">
            <div className="relative flex-1 bg-gray-950">
                {remoteStreamID ? (
                    <div id={`psycho-remote-${remoteStreamID}`} className="absolute inset-0 w-full h-full">
